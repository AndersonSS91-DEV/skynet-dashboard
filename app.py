...
# ===============================
# MONTE CARLO INSTITUCIONAL (FINAL LIMPO)
# ===============================
...
for _ in range(simulacoes):
    ...
    for r in seq:
        ...
        if len(curvas) < 150:
            curva.append(banca / banca_inicial)  # <<< MULTIPLOS

    resultados_finais.append(banca / banca_inicial)  # <<< MULTIPLOS

    if len(curvas) < 150:
        curvas.append(curva)

# ===============================
# ESTATÍSTICAS (EM X DA BANCA)
# ===============================
...
# ===============================
# CARDS (AGORA FAZ SENTIDO)
# ===============================

c1, c2, c3, c4 = st.columns(4)

c1.metric("P10 (pior cenário)", f"{p10:.1f}x")
c2.metric("Mediana (P50)", f"{p50:.1f}x")
c3.metric("P90 (top cenário)", f"{p90:.1f}x")
...
fig.update_layout(
    template="plotly_dark",
    title="Distribuição da Banca Final (em X)"
)
...
fig_curvas.update_layout(
    template="plotly_dark",
    title="Curvas Simuladas (múltiplos da banca)"
)
